...
public class Soccer {
    ...
    Player p, p2, p3, q, q2, q3, r, selected;		// 플레이어
    Vector<Player> pVec = new Vector();
    Vector<Player> qVec = new Vector();
    boolean timeout, goal, gameOver, pause;		// 각각 term과 골인 여부 판단
    int selectIndex =0;
    ...
    Soccer()
    {
        ...
        pVec.add(new Player("손", "P", JField, -50, 10));	// (-50, 10)에 선수 생성)
        pVec.add(new Player("송", "P", JField, -100, 60));
        pVec.add(new Player("신", "P", JField, -100, -40));
        /*p = new Player("손", "P", JField, -50, 10);	// (-50, 10)에 선수 생성
        p2 = new Player("송", "P", JField, -100, 60);
        p3 = new Player("신", "P", JField, -100, -40);*/
        qVec.add(new Player("기", "Q", JField, 50, -10));
        qVec.add(new Player("강", "Q", JField, 100, -60));
        qVec.add(new Player("강", "Q", JField, 100, 40));
        /*q = new Player("기", "Q", JField, 50, -10);	// (50, 10)에 선수 생성
        q2 = new Player("강", "Q", JField, 100, -60);
        q3 = new Player("강", "Q", JField, 100, 40);*/

        selected = qVec.elementAt(selectIndex);
        selected.choosen = true;
    ...
    void Run()
    {
        while(!timeout)
        {
            ...
            for(int i =0 ; i < pVec.size(); i++){
                if(i==0) {
                    dist = pVec.elementAt(i).Move(ball);
                    r = pVec.elementAt(i);
                    continue;
                }
                else{
                    int d =pVec.elementAt(i).Move(ball);
                    if(d < dist) {
                        dist = d;
                        r = pVec.elementAt(i);
                    }
            ...
            for(int i= 0; i < qVec.size(); i++){
                int d = qVec.elementAt(i).Move(ball);
                if(d < dist){
                    dist = d;
                    r = qVec.elementAt(i);
                }
            ...
            if(goal)
            {
                ...
                // 공 및 선수의 상태를 초기화시킴
                ball.InitBall();
                pVec.elementAt(0).InitPlayerState(-50, 10);
                pVec.elementAt(1).InitPlayerState(-100, 60);
                pVec.elementAt(2).InitPlayerState(-100, -40);
                qVec.elementAt(0).InitPlayerState(50, -10);
                qVec.elementAt(1).InitPlayerState(100, 40);
                qVec.elementAt(2).InitPlayerState(100, -60);
            }
...
class JField extends JPanel {
        ...
        public void keyPressed(KeyEvent e) {
            ...
            switch(e.getExtendedKeyCode()) {
                case KeyEvent.VK_SPACE:
                    ...
                    break;
                case KeyEvent.VK_S:
                    match.ChangeSelectedPlayer();
                    break;
                case KeyEvent.VK_RIGHT:
                    if(match.pause)
                        break;
                    //System.out.println("q run!!");
                    match.selected.runtoward(2*speed, 0);
                    break;
                ...
                case KeyEvent.VK_UP:
                    if(match.pause)
                        break;
                    //System.out.println("q run!!");
                    match.selected.runtoward(0, -2*speed);
                    break;
    ...
    public void paint(Graphics g)
    {
        ...
        for(int i = 0 ; i < match.pVec.size() ; i++){
            match.pVec.elementAt(i).draw(g);
        }
        g.setColor(Color.blue); //match.q.draw(g);
        for(int i = 0 ; i < match.qVec.size() ; i++){
            match.qVec.elementAt(i).draw(g);
        }
    }
    public void drawClock(Graphics g) {
        int r = 10, x = w-r-10,  y = r+10;
        double a = Math.PI/180.0 * ((double)-90 + (double)match.clock*(360.0/900.0));
    ...
    void drawScore(Graphics g) {
        ...
        g.drawString(Integer.toString(match.leftScore), this.getCx()-15, this.getCy() - y1+15);
        g.drawString(Integer.toString(match.rightScore), this.getCx()+15, this.getCy() - y1+15);
    }
}
...
class Player {
    ...
    public void draw(Graphics g) {
        ...
        if(this.choosen){
            g.fillRect(JField.getCx() + x -5, JField.getCy() + y-20, 10, 20);
        }
    }
